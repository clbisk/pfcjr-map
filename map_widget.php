...
class Map_Widget extends Widget_Base {
	...
	private static $state_acronyms = array("NC" => 'North Carolina', "TN" => 'Tennessee', "RI" => "Rhode Island", "CT" => "Connecticut", "MA" => "Massachusetts", "ME" => "Maine", "NH" => "New Hampshire", "VT" => "Vermont", "NY" => "New York", "NJ" => "New Jersey", "PA" => "Pennsylvania", "DE" => "Delaware", "MD" => "Maryland", "WV" => "West Virginia", "KY" => "Kentucky", "OH" => "Ohio", "MI" => "Michigan", "WY" => "Wyoming", "MT" => "Montana", "ID" => "Idaho", "WA" => "Washington", "TX" => "Texas", "CA" => "California", "AZ" => "Arizona", "NV" => "Nevada", "UT" => "Utah", "CO" => "Colorado", "NM" => "New Mexico", "OR" => "Oregon", "ND" => "North Dakota", "SD" => "South Dakota", "NE" => "Nebraska", "IA" => "Iowa", "MS" => "Mississippi", "IN" => "Indiana", "IL" => "Illinois", "MN" => "Minnesota", "WI" => "Wisconsin", "MO" => "Missouri", "AR" => "Arkansas", "OK" => "Oklahoma", "KS" => "Kansas", "LA" => "Louisiana", "VA" => "Virginia", "HI" => "Hawaii", "FL" => "Florida", "SC" => "South Carolina", "GA" => "Georgia", "AL" => "Alabama", "AK" => "Alaska");
	private static $state_polys = array("HI" => "M407.1,619.3l1.9-3.6l2.3-0.3l0.3,0.8l-2.1,3.1H407.1z M417.3,615.6l6.1,2.6l2.1-0.3l1.6-3.9   l-0.6-3.4l-4.2-0.5l-4,1.8L417.3,615.6z M448,625.6l3.7,5.5l2.4-0.3l1.1-0.5l1.5,1.3l3.7-0.2l1-1.5l-2.9-1.8l-1.9-3.7l-2.1-3.6   l-5.8,2.9L448,625.6z M468.2,634.5l1.3-1.9l4.7,1l0.6-0.5l6.1,0.6l-0.3,1.3l-2.6,1.5l-4.4-0.3L468.2,634.5z M473.5,639.7l1.9,3.9   l3.1-1.1l0.3-1.6l-1.6-2.1l-3.7-0.3V639.7z M480.5,638.5l2.3-2.9l4.7,2.4l4.4,1.1l4.4,2.7v1.9l-3.6,1.8l-4.8,1l-2.4-1.5   L480.5,638.5z M497.1,654.1l1.6-1.3l3.4,1.6l7.6,3.6l3.4,2.1l1.6,2.4l1.9,4.4l4,2.6l-0.3,1.3l-3.9,3.2l-4.2,1.5l-1.5-0.6l-3.1,1.8   l-2.4,3.2l-2.3,2.9l-1.8-0.2l-3.6-2.6l-0.3-4.5l0.6-2.4l-1.6-5.7l-2.1-1.8l-0.2-2.6l2.3-1l2.1-3.1l0.5-1l-1.6-1.8L497.1,654.1z",
    ...
    function __construct($data = [], $args = null) {
		...
		wp_register_script('bootstrap', '//cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/js/bootstrap.bundle.min.js', array('jquery'), '5.1.3', true);
		
		$my_script_dependencies = array('jquery', 'elementor-frontend', 'bootstrap');
		wp_register_script('interactive_map', plugins_url('interactive_map.js', __FILE__),
		$my_script_dependencies, '1.1', true);

		wp_register_style('bootstrap-css', '//cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css');
	...
	public function get_script_depends() {
		return array( 'bootstrap', 'interactive_map' );
	}
	...
	private function _escape_quotation_marks($string) {
		$string = preg_replace('/"(>.*<\/a>)/', "'$1", $string);
		$string = preg_replace('/(<a.*)"/', "$1'", $string);
	...
	private function _add_state_control($state) {
		...
		$supported_repeater->add_control(
			...
			[
				'label' => __( 'Supported Policy Name', self::$map_widget_name ),
		...
		$opposed_repeater->add_control(
			...
			[
				'label' => __( 'Opposed Policy Name', self::$map_widget_name ),
				'type' => Controls_Manager::TEXT,
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::REPEATER,
				'prevent_empty' => false,
				'fields' => $opposed_repeater->get_controls(),
				'default' => [],
	...
	protected function _register_controls() {
		...
		foreach (self::$state_acronyms as $state_acronym => $state_name) {
			self::_add_state_control($state_name);
		}
	...
	private function _render_policy($state_name, $state_pro_policies, $state_anti_policies, $state_action_items) {
		...
		if (sizeof($state_pro_policies) > 0) {
			...
			foreach ($state_pro_policies as $pro_policy) {
				...
				$state_pro_policy = self::_escape_quotation_marks($raw_state_pro_policy);
				$raw_state_pro_policy_info = $pro_policy[$state_name . '_supported_policy_info'];
				$state_pro_policy_info = self::_escape_quotation_marks($raw_state_pro_policy_info);
		...
		if (sizeof($state_anti_policies) > 0) {
			...
			foreach ($state_anti_policies as $anti_policy) {
				...
				$state_anti_policy = self::_escape_quotation_marks($raw_state_anti_policy);
				$raw_state_anti_policy_info = $anti_policy[$state_name . '_opposed_policy_info'];
				$state_anti_policy_info = self::_escape_quotation_marks($raw_state_anti_policy_info);
		...
		if (sizeof($state_action_items) > 0) {
			...
			foreach ($state_action_items as $state_action_item) {
				...
				$action_item = self::_escape_quotation_marks($raw_action_item);
    ...
    protected function render() {
		...
		<svg xmlns:cc="http://creativecommons.org/ns#" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#" xmlns:inkscape="http://www.inkscape.org/namespaces/inkscape" xmlns:svg="http://www.w3.org/2000/svg" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" id="us-map" preserveAspectRatio="xMinYMin meet" sodipodi:docname="Republican_Party_presidential_primaries_results,_2016.svg" inkscape:version="0.91 r13725" x="0px" y="0px" width="959px" height="593px" viewBox="174 100 959 593" enable-background="new 174 100 959 593" xml:space="preserve">
		...
		if (sizeof($dc_pro_policies) > 0) {
			...
			foreach ($dc_pro_policies as $pro_policy) {
				...
				$dc_pro_policy = self::_escape_quotation_marks($raw_dc_pro_policy);
				$raw_dc_pro_policy_info = $pro_policy['DC_supported_policy_info'];
				$dc_pro_policy_info = self::_escape_quotation_marks($raw_dc_pro_policy_info);
		...
		if (sizeof($dc_anti_policies) > 0) {
			...
			foreach ($dc_anti_policies as $anti_policy) {
				...
				$dc_anti_policy = self::_escape_quotation_marks($raw_dc_anti_policy);
				$raw_dc_anti_policy_info = $anti_policy['DC_opposed_policy_info'];
				$dc_anti_policy_info = self::_escape_quotation_marks($raw_dc_anti_policy_info);
		...
		if (sizeof($dc_action_items) > 0) {
			...
			foreach ($dc_action_items as $action_item) {
				...
				$dc_action_item = self::_escape_quotation_marks($raw_dc_action_item);
